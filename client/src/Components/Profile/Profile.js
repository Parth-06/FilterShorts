  ...
  return (
    ...
    <div className='profile'>
      ...
      <div className="profiledetails">
        ...
        <img src={userdata.profilepicimage} className="profileavatar" alt=''  style={{objectFit:"cover"}}/>
        <div className="profilename">
          <h1>{userdata.name}</h1>
          <h4>{userdata.username}</h4>
          <button className='profilebtn' onClick={()=>navigate("/editprofile")}>Edit Profile</button>
        </div>
      </div>
      <div className="followinfo">
      {       userdata.followers === undefined ?
                   (<>0 <p onClick={()=>navigate("/followersmain")}>Followers</p></>):
                   (<>{userdata.followers.length} <p onClick={()=>navigate("/followersmain")}>Followers</p></>)}
                 
                 
                 
                  { userdata.following === undefined ?
                   (<>0 <p onClick={()=>navigate("/followingmain")}>Following</p></>):
                   (<>{userdata.following.length}  <p onClick={()=>navigate("/followingmain")}>Following</p></>)}
